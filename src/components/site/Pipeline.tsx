...
export function Pipeline() {
  ...
  const activeIdx = Math.min(STAGES.length - 1, Math.floor(t * STAGES.length));
  const ms = (11.4 * t).toFixed(2);

  return (
    <section id="routing" className="section" style={{ paddingBottom: 0 }}>
      ...
      <div ref={wrap} className="pipeline-pin">
        <div className="sticky">
          <div className="container-x">
            <div className="pipeline" style={{
              transform: `perspective(1400px) rotateX(${(0.5 - t) * 4}deg)`,
              ...
              <div className="pipeline-grid">
                {STAGES.map((s, i) => (
                  <div
                    key={i}
                    className={`pipeline-stage ${i <= activeIdx ? "is-active" : ""}`}
                    style={{
                      transform: `translateY(${(i - activeIdx) * -2}px)`,
                      transition: "transform .5s var(--ease-out)",
                    }}
                  >
                    <div className="num-big">{s.idx}</div>
                    <div>
                ...
                <div className="pipeline-token" style={{
                  left: `${t * 100}%`,
              ...
              <div style={{
                ...
                color: "#9aa0a8", textTransform: "uppercase",
              }}>
                <span>T+0.00ms · Request</span>
                <span>T+{ms}ms · {STAGES[activeIdx].ttl}</span>
