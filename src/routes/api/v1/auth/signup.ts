      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          await logEvent(
            data.user.id,
            USER_SIGNUP,
            { email: data.user.email, name, userId: data.user.id },
            ipAddress,
          );
          ...
          return Response.json(
            { success: false, error: "Internal Server Error", message: "An unexpected error occurred during registration. Please try again later." },
            { status: 500 },
          );
        }
      },
