...
// 🔒 Redis-backed rate limit — survives across Cloudflare Worker instances and cold starts.
// The previous in-memory Map was silently reset on every isolate restart, making
// rate limiting completely ineffective in serverless/edge deployments.
...
export async function generateApiKey(environment: KeyEnvironment = 'live'): Promise<GeneratedApiKey> {
  const entropy = new Uint8Array(32);
  crypto.getRandomValues(entropy);
  const rawSecret = Array.from(entropy).map(b => b.toString(16).padStart(2, '0')).join('');
...
export async function validateApiKey(
  ...
  const rateLimitResult = await checkRateLimit(record.key_prefix, record.environment);
...
/**
 * Checks and increments the rate limit counter for an API key in Redis.
 * Uses a sliding 60-second window stored as a Redis key with TTL.
 * Falls back to allowing the request if Redis is unavailable (fail open).
 */
export async function checkRateLimit(
  keyPrefix: string,
  environment: KeyEnvironment
): Promise<{ blocked: boolean; calls: number }> {
  const limit = RATE_LIMITS[environment];
  const redis = getRedisClient();

  if (!redis) {
    // Redis not configured — fail open with a warning
    console.warn('[ApiKeyManager] Redis not configured, rate limiting disabled');
    return { blocked: false, calls: 0 };
  }

  const key = `rl:apikey:${keyPrefix}`;
  try {
    const pipeline = redis.pipeline();
    pipeline.incr(key);
    pipeline.expire(key, 60); // 60-second sliding window
    const [count] = await pipeline.exec() as [number, number];
    const calls = typeof count === 'number' ? count : 1;
    return { blocked: calls > limit, calls };
  } catch (err) {
    console.error('[ApiKeyManager] Redis rate limit check failed, failing open:', err);
    return { blocked: false, calls: 0 };
  }
}

export function getRateLimitState(_keyPrefix: string): RateLimitState | null {
  // Deprecated: in-memory state is no longer used. Returns null.
  return null;
}

export function resetRateLimit(_keyPrefix: string): void {
  // Deprecated: use Redis TTL expiry instead.
}
...
export async function rotateApiKey(
  ...
  const newGenerated = await generateApiKey(old.environment);
