...
import { classifyTraffic } from './ml/classifier';
 ...
 */
async function checkRateLimit(ip: string, now: number, traceCtx: any): Promise<boolean> {
  try {
    ...
    emitTelemetryEvent({ ...traceCtx, type: 'FailOpen', subsystem: 'redis', error: err.message || 'Rate limit check failed' });
    return true;
  }
}
 ...
 */
async function getLinkConfig(slug: string, traceCtx: any): Promise<LinkConfig | null> {
  try {
    const redis = getRedis();
    ...
    emitTelemetryEvent({ ...traceCtx, type: 'FailOpen', subsystem: 'redis', error: err.message || 'Config lookup failed' });
    return null;
  }
}
...
export async function middleware(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    emitTelemetryEvent({ ...traceCtx, type: 'FailOpen', subsystem: 'ml', error: mlErr.message || 'ML Classification failed' });
  }
  ...
  try {
    linkConfig = await getLinkConfig(slug, traceCtx);
  } catch (kvErr) {
